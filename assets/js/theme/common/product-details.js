...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        this.$overlay = $('[data-cart-item-add] .loadingOverlay, [data-cart-item-add-bc] .loadingOverlay');
        ...
        const $form = $('form[data-cart-item-add], form[data-cart-item-add-bc]', $scope);
        ...
        const $productOptionsElement = $('[data-product-option-change], [data-product-option-change-bc]', $form);
        ...
        const stickyOptionElement = $('[data-product-option-change-bc]');
        if (stickyOptionElement.html().trim().length) {
            stickyOptionElement.show();
        } else {
            stickyOptionElement.hide();
        }
    ...
    setProductVariantBc() {
        ...
        $.each($('[data-product-option-change-bc] [data-product-attribute]'), (index, value) => {
            const optionLabel = value.children[0].innerText;
            const optionTitle = optionLabel.split(':')[0].trim();
            const required = optionLabel.toLowerCase().includes('required');
            const type = value.getAttribute('data-product-attribute');
    ...
    syncSelectedOptions() {
        $('div[data-product-option-change] input[type="radio"], div[data-product-option-change-bc] input[type="radio"]').on('change', function() {
            const selectedValue = $(this).val();
    
            if ($(this).closest('div[data-product-option-change]').length) {
                $(`div[data-product-option-change-bc] input[type="radio"][value="${selectedValue}"]`).prop('checked', true);
            } else {
                $(`div[data-product-option-change] input[type="radio"][value="${selectedValue}"]`).prop('checked', true);
            }
        });
    }
    ...
    updateProductDetailsData() {
        const $form = $('form[data-cart-item-add], form[data-cart-item-add-bc]');
